...
export default function PlanilhaScreen({ route }) {
  ...
  async function salvarEdicaoFixa(id) {
    setDespesasFixas(
      despesasFixas.map((d) =>
        d.id === id ? { ...d, nome: nomeEdit, valor: Number(valorEdit) } : d,
      ),
    );
    setEditando(null);
  }
  ...
  async function adicionarFixa() {
    ...
    try {
      const nova = await createDespesaFixa({
  ...
  async function adicionarExtra() {
    ...
    try {
      const hoje = new Date();
      const nova = await createDespesaExtra({
        usuarioId: usuario.id,
        nome,
        valor: Number(valor),
  ...
  async function removerFixa(id) {
    try {
      const sucesso = await deleteDespesaFixa(id);
      if (sucesso) {
  ...
  async function removerExtra(id) {
    try {
      const sucesso = await deleteDespesaExtra(id);
      if (sucesso) {
        setDespesasExtras(despesasExtras.filter((d) => d.id !== id));
      }
    } catch (error) {
      console.error("Erro ao remover despesa extra:", error);
    }
  }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.resumoCard}>
        <View style={styles.resumoRow}>
          <View>
            ...
            <Text style={styles.resumoValor}>
              R$ {Number(usuario.salario).toFixed(2)}
            </Text>
          </View>
        ...
        <View style={styles.barraFundo}>
          <View
            style={[styles.barraPreenchimento, { width: `${barWidth}%` }]}
          />
        </View>
        <Text style={styles.pctTexto}>{pct}% do salário comprometido</Text>
        <View style={styles.totalRow}>
          <View style={styles.totalItem}>
            <Text style={styles.totalLabel}>Fixas</Text>
            <Text style={styles.totalValor}>R$ {totalFixas.toFixed(2)}</Text>
          </View>
          <View style={styles.totalSep} />
          <View style={styles.totalItem}>
            <Text style={styles.totalLabel}>Extras</Text>
            <Text style={styles.totalValor}>R$ {totalExtras.toFixed(2)}</Text>
          </View>
          <View style={styles.totalSep} />
          <View style={styles.totalItem}>
            <Text style={styles.totalLabel}>Total</Text>
            <Text style={[styles.totalValor, { color: "#F87171" }]}>
      ...
      <View style={styles.abaRow}>
        <TouchableOpacity
          style={[styles.aba, abaAtiva === "fixas" && styles.abaAtiva]}
          onPress={() => setAbaAtiva("fixas")}
        >
          <Text
            style={[
              styles.abaTexto,
              abaAtiva === "fixas" && styles.abaTextoAtivo,
            ]}
          >
        ...
        <TouchableOpacity
          style={[styles.aba, abaAtiva === "extras" && styles.abaAtiva]}
          onPress={() => setAbaAtiva("extras")}
        >
          <Text
            style={[
              styles.abaTexto,
              abaAtiva === "extras" && styles.abaTextoAtivo,
            ]}
          >
            Extras ({despesasExtras.length})
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {abaAtiva === "fixas" && (
        <>
                ...
                <>
                  ...
                  <View style={styles.linhaBotoes}>
                    <TouchableOpacity
                      ...
                      onPress={() => iniciarEdicao(d)}
                    >
                      <Text>✏️</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={styles.btnRemover}
                      onPress={() => removerFixa(d.id)}
                    >
          ...
          <TouchableOpacity style={styles.addBotao} onPress={adicionarFixa}>
            <Text style={styles.addBotaoTexto}>+ Adicionar Fixa</Text>
          </TouchableOpacity>
        </>
      )}
      ...
      {abaAtiva === "extras" && (
        <>
          ...
          <TouchableOpacity style={styles.addBotao} onPress={adicionarExtra}>
            <Text style={styles.addBotaoTexto}>+ Adicionar Extra</Text>
          </TouchableOpacity>
        </>
      )}
