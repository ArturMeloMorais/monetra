...
import { getDespesasByUsuario } from "../services/backend";
...
export default function PlanilhaScreen({ route }) {
  ...
  useEffect(() => {
    async function carregarDespesas() {
      const dadosUsuario = await getDespesasByUsuario(usuario.id);
      setDespesasFixas(dadosUsuario.despesasFixas || []);
      setDespesasExtras(dadosUsuario.despesasExtras || []);
    }
    carregarDespesas();
  }, [usuario.id]);
  ...
  const totalFixas = despesasFixas.reduce(
    (soma, d) => soma + Number(d.valor),
    0,
  );
  const totalExtras = despesasExtras.reduce(
    (soma, d) => soma + Number(d.valor),
    0,
  );
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.salario}>Salário: R$ {usuario.salario}</Text>
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 20, backgroundColor: "#1c7c43" },
  ...
  titulo: { fontSize: 16, fontWeight: "bold", marginTop: 10, color: "white" },
  ...
  botaoTexto: { color: "black", fontWeight: "bold" },
